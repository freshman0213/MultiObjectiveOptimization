...
class MNIST(data.Dataset):
    ...
    def __init__(self, root, split='train', transform=None, download=False):
        ...
        if self.split == 'train':
            ...
            self.val_data, self.val_labels_l, self.val_labels_r = torch.load(
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            img, target_l, target_r = self.val_data[index], self.val_labels_l[index], self.val_labels_r[index]
        else:
    ...
    def _check_multi_exists(self):
        return  os.path.exists(os.path.join(self.root, self.processed_folder, self.multi_training_file)) and \
            os.path.exists(os.path.join(self.root, self.processed_folder, self.multi_validation_file)) and \
    ...
    def download(self):
        ...
        permutation = np.random.permutation(multi_mnist_ims.size(dim=0))
        training_indices = permutation[:48000]
        validation_indices = permutation[48000:]
        multi_mnist_training_set = (multi_mnist_ims[training_indices], multi_mnist_labels_l[training_indices], multi_mnist_labels_r[training_indices])
        multi_mnist_validation_set = (multi_mnist_ims[validation_indices], multi_mnist_labels_l[validation_indices], multi_mnist_labels_r[validation_indices])
