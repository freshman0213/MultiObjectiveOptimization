...
def train_multi_task_scalarization(param_file):
    RESULT_FOLDER = './results/celeba_film/'
    ...
    for trial_idx in range(num_trials):
        ...
        # if not os.path.exists("./saved_models/baseline/{}_model.pkl".format(trial_identifier)):
            # Trian the multi-task model with specific params
            # train_multi_task(trial_params)

        # Retrieve the best model's performance
        if not os.path.exists("./saved_models/{}_model.pkl".format(trial_identifier)):  # CHANGE PATH
            continue
        tasks = fixed_params['tasks']
        state = torch.load("./saved_models/{}_model.pkl".format(trial_identifier))  # CHANGE PATH
        testing_metric = {}
        testing_loss = {}
        for t in tasks:
        ...
        non_dominated = True
        for x in pareto_frontier_acc:
            if pareto_dominate(x, testing_metric):
                non_dominated = False 
                break
        if not non_dominated:
            continue
            ...
            for x in pareto_frontier_acc:
                if pareto_dominate(testing_metric, x):
                    pareto_frontier_acc.remove(x)
            pareto_frontier_acc.append(testing_metric)
    ...
    np.save(exp_identifier+'_acc', pareto_frontier_acc)
